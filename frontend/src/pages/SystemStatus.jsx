...
import { Database, Bot, MessageSquare, Server, CheckCircle2, XCircle } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-6">
        ...
        <div className={`px-2.5 py-1 rounded-full text-xs font-medium flex items-center gap-1.5 border ${isOk ? 'bg-success/10 text-success border-success/20' : 'bg-danger/10 text-danger border-danger/20'}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${isOk ? 'bg-success' : 'bg-danger'}`}></span>
          {status}
        </div>
      </div>
